...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  G4double RPCXFirstLayerPos = fDetector->GetRPCRadioPos()*cos(fDetector->GetRPCPhiPos());
  G4double RPCZFirstLayerPos = -1*fDetector->GetRPCRadioPos()*sin(fDetector->GetRPCPhiPos());

  //if (volName != "World" && volName != "Absorber"){G4cout<<volName<<G4endl;}

  ////// //////		Secondary particle produced		////// ///////
  /*if ( (volName == "World")  && (parentID != 0) ) {//&& (edep > 0) ) {
  	  if (particleName == "neutron") { analysisManager->FillH1(4,edep,1.);}
  	  if (particleName == "gamma") { analysisManager->FillH1(5,edep,1.);}
  }*/
  ...
  if ( ((volName == "fakeBottom") || (volName == "fakeTop")) && (p_incident == 0) ) {    // particella incidente o sul falso layer top o bottom (per contare numero particelle incidenti)
	  ...
	  particle_incident_TOT.z 	= 	step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;							//posizione sulla superficie top sulla quale incidono particelle
	  particle_incident_TOT.x 	= 	step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
  ...
  if ( (volName == "fakeTop") && (p_incident_top == 0) && (p_incident_bot == 0) ) {    // particella incidente  sul falso layer top (per contare numero particelle incidenti)
	  ...
	  particle_incident_TOP.z 	= 	step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;							//posizione sulla superficie top sulla quale incidono particelle
	  particle_incident_TOP.x 	= 	step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
  ...
  if ( (volName == "fakeBottom") && (p_incident_top == 0) && (p_incident_bot == 0) ) {    	// particella incidente  sul falso layer top (per contare numero particelle incidenti)
	  ...
	  particle_incident_BOT.z 	= 	step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;							//posizione sulla superficie top sulla quale incidono particelle
	  particle_incident_BOT.x 	= 	step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
  ...
  if ( ((volName == "GasGap1") || (volName == "GasGap2")) && (particleCharge != 0) && (edep > 0) && (p_charge == 0) /*&& (p_incident == 1)*/ ) {  // particella carica e che ha depositato energia in gap 1 o gap 2
	  ...
	  particle_charge_TOT.z		=	 step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;
	  particle_charge_TOT.x		=	 step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
	  ...
	  particle_charge_TOT.x_creation	=	posizione_creazione_TOT[0] - RPCXFirstLayerPos;		//posizione sulla superficie gap1 sulla quale si crea particella carica (getvertexposition)
	  particle_charge_TOT.y_creation	=	posizione_creazione_TOT[1];
	  particle_charge_TOT.z_creation	=	posizione_creazione_TOT[2] - RPCZFirstLayerPos;
  ...
  if ( (volName == "GasGap1") && (particleCharge != 0) && (p_charge_gap1 == 0) && (edep > 0) && (p_charge_gap2 == 0) /*&& (p_incident == 1)*/ ) {
  	  ...
  	  particle_charge_Gap1.z		=	 step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;
  	  particle_charge_Gap1.x		=	 step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
  	  ...
  	  //ricava informazioni (posizione e processo) da cui si è formata particella carica che ha interagito
  	  G4ThreeVector posizione_creazione_TOT= step->GetTrack()->GetVertexPosition();    // vettore prende posizione
  	  particle_charge_Gap1.x_creation	=	posizione_creazione_TOT[0] - RPCXFirstLayerPos;		//posizione sulla superficie gap1 sulla quale si crea particella carica (getvertexposition)
  	  particle_charge_Gap1.y_creation	=	posizione_creazione_TOT[1];
  	  particle_charge_Gap1.z_creation	=	posizione_creazione_TOT[2] - RPCZFirstLayerPos;
    ...
    if ( (volName == "GasGap2") && (particleCharge != 0) && (p_charge_gap1 == 0) && (edep >0) && (p_charge_gap1 == 0) /*&& (p_incident == 1)*/ ) {  // particella carica e che ha depositato energia in gap 1 o gap 2
    	...
    	particle_charge_Gap2.z		=	 step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;
    	particle_charge_Gap2.x		=	 step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
    	...
    	//ricava informazioni (posizione e processo) da cui si è formata particella carica che ha interagito
    	G4ThreeVector posizione_creazione_TOT= step->GetTrack()->GetVertexPosition();    // vettore prende posizione
    	particle_charge_Gap2.x_creation	=	posizione_creazione_TOT[0] - RPCXFirstLayerPos;		//posizione sulla superficie gap1 sulla quale si crea particella carica (getvertexposition)
    	particle_charge_Gap2.y_creation	=	posizione_creazione_TOT[1];
    	particle_charge_Gap2.z_creation	=	posizione_creazione_TOT[2] - RPCZFirstLayerPos;
...

  {
	  ...
	  particleKODELStep.z	=	step->GetTrack()->GetPosition().z() - RPCZFirstLayerPos;					//posizione sulla superficie top sulla quale incidono particelle
	  particleKODELStep.x	=	step->GetTrack()->GetPosition().x() - RPCXFirstLayerPos;
