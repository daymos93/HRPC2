...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  if ( ((volName == "fakeBottom") || (volName == "fakeTop")) && (p_incident == 0) ) {    // particella incidente o sul falso layer top o bottom (per contare numero particelle incidenti)
	  ...
	  particle_incident_TOT.z 	= 	step->GetTrack()->GetPosition().z() ;							//posizione sulla superficie top sulla quale incidono particelle
	  particle_incident_TOT.x 	= 	step->GetTrack()->GetPosition().x() ;
